...
<body>
	...
	String mensaje = (String) session.getAttribute("mensaje");	//Cogemos el mensaje específico para cada situación
	...
	<table> <!-- Construimos el mar de juego -->
		<tr>
		<td></td>
		...
		<%for(int i = 0; i < partida.getFilas(); i++){%>
			...
			for(int j = 0; j <= partida.getColumnas(); j++){
				...
				<%}
					...
					if(partida.casillaDisparada(i, j-1)){//Actualizamos segun el estado de la casilla%>
	...
	<%if(quedan == 0){	//Deshabilitamos el botón cuando hemos terminado la partida %> 
