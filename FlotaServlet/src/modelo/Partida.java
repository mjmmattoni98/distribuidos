...
public class Partida {
	...
	private void iniciaDisparos(int filas, int columnas) {
		this.misDisparos = new boolean[filas][columnas];
	}
	
	public boolean casillaDisparada(int fila, int columna) {
		return this.misDisparos[fila][columna];
	}
	
	public int getCasilla(int fila, int columna) {
		return this.mar[fila][columna];
	}
    ...
    public int pruebaCasilla(int f, int c) {
    	...
    	if(estado > -1) { //Entramos si tocamos algún barco
        	Barco barco = barcos.get(estado);
        	barco.tocaBarco();
        	
        	if(barco.estaHundido()) {
        		int filaInicial = barco.getFilaInicial();
        		int columnaInicial = barco.getColumnaInicial();
        		char orientacion = barco.getOrientacion(); 
        		for(int i = 0; i < barco.getTamanyo(); i++) {
        			if(orientacion == 'H')	//Orientación horizontal
        				mar[filaInicial][columnaInicial + i] = HUNDIDO;
        			else 							//Orientación vertical
        				mar[filaInicial + i][columnaInicial] = HUNDIDO;
        		}
        		quedan--; //decrementar barcos restantes
        		return estado; //ID barco
        	}
        	mar[f][c] = TOCADO;
        	return TOCADO;
        }
    	else
    		return estado; //Devuelve el estado de la casilla (AGUA, TOCADO, HUNDIDO) porque no es necesario cambiarlo
	...
	public String[] getSolucion() {
		String[] stringBarcos = new String[numBarcos];
		for(int i = 0; i < numBarcos; i++) 
			stringBarcos[i] = getBarco(i);
	...
	private void ponBarcos() {
		/* Por defecto colocamos un barco de tama�o 4, uno de tama�o 3, otro de tama�o 2 y tres barcos de tama�o 1 */
